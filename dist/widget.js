...
`;function _(n){let e=n.replace(/\D+/g,"");e.startsWith("0")&&(e=e.slice(1)),e=e.slice(0,9);let a="";return e.length>0&&(a+=e.slice(0,2)),e.length>2&&(a+=" "+e.slice(2,5)),e.length>5&&(a+=" "+e.slice(5,7)),e.length>7&&(a+=" "+e.slice(7,9)),a}function u(n){if(!n)throw new Error("RA360Widget: host element not found");const e=n.shadowRoot||n.attachShadow({mode:"open"});e.innerHTML=k;const a=e,d=a.getElementById("accompagnement"),r=a.getElementById("accompagnement-expander"),l=a.getElementById("accompagnement-form"),E=a.querySelector(".ra-badges"),z=a.getElementById("raison-list"),I=a.getElementById("acf-code"),p=a.getElementById("acf-phone"),g=a.getElementById("ra-success");y.forEach(t=>{const i=document.createElement("span");i.className="ra-badge",i.textContent=t,E.appendChild(i);const c=document.createElement("option");c.value=t,z.appendChild(c)});const h=()=>{p.value=_(p.value)};["input","blur","paste"].forEach(t=>p.addEventListener(t,h));function R(){r.setAttribute("aria-hidden","false"),d.setAttribute("aria-expanded","true"),r.style.height=r.scrollHeight+"px",r.scrollIntoView({behavior:"smooth",block:"start"})}function S(){r.setAttribute("aria-hidden","true"),d.setAttribute("aria-expanded","false"),r.style.height="0px"}function m(){r.getAttribute("aria-hidden")==="false"?S():R()}d.addEventListener("click",m),d.addEventListener("keydown",t=>{(t.key==="Enter"||t.key===" ")&&(t.preventDefault(),m())}),new ResizeObserver(()=>{r.getAttribute("aria-hidden")==="false"&&(r.style.height=r.scrollHeight+"px")}).observe(r);function B(){g.classList.add("show"),r.style.height=r.scrollHeight+"px",g.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>{g.classList.remove("show"),r.style.height=r.scrollHeight+"px"},6e3)}l.addEventListener("submit",async t=>{if(t.preventDefault(),!l.reportValidity())return;h();const i=I.value||"+41",c=p.value.trim(),C=`${i} ${c}`.trim(),o=l.querySelector(".ra-btn"),b=o.textContent;o.disabled=!0,o.textContent="Envoi…";const M={firstname:a.getElementById("acf-firstname").value.trim(),phone:C,raison:a.getElementById("acf-raison").value.trim(),whatsapp:a.getElementById("wa").checked,consent:a.getElementById("c").checked,ua:navigator.userAgent};try{const f=await fetch(w,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(M)});if(!f.ok)throw new Error(await f.text());l.reset(),o.textContent="Reçu ✔",B(),setTimeout(()=>{o.textContent=b,o.disabled=!1},1600)}catch(f){console.error("RA360Widget: lead error",f),alert("Oups, une erreur est survenue. Réessayez plus tard."),o.disabled=!1,o.textContent=b}})}function A(n="#ra"){const e=document.querySelector(n);if(!e)throw new Error(`RA360Widget: selector not found: ${n}`);u(e)}function x(n){u(n)}return typeof window!="undefined"&&(window.RAInit=x),s.RAInit=x,s.init=A,Object.defineProperty(s,Symbol.toStringTag,{value:"Module"}),s}({});
